...
window = pyglet.window.Window(width=800, height=800, resizable=True, vsync=False)
...
def on_text_motion(motion):
    global mode, dm, vertex_list, scene
...
def create_shader(strings, _type):
    print(''.join(strings))
    source = bytes(''.join(strings), 'utf-8')
    c_source = cffi.create_string_buffer(source)
    c_source_ptr = cffi.cast(cffi.pointer(c_source), cffi.POINTER(cffi.c_char))
    shader = glCreateShader(_type)
    glShaderSource(shader, 1, cffi.byref(c_source_ptr), None)
    glCompileShader(shader)

    temp = cffi.c_int(0)
    glGetShaderiv(shader, GL_COMPILE_STATUS, cffi.byref(temp))

    if not temp:
        glGetShaderiv(shader, GL_INFO_LOG_LENGTH, cffi.byref(temp))
...
vert_shader = create_shader(ff, GL_VERTEX_SHADER)

ff = read_source('./shaders/PhongLight.frag')
frag_shader = create_shader(ff, GL_FRAGMENT_SHADER)
